# Scientific Reports

Public experiment reports follow the
[scientific report standard](../standards/scientific-report-standard.md) and
[authoring protocol](../standards/scientific-report-authoring-protocol.md).
Start from [the template](templates/scientific-experiment-report.md) and pair
each accepted report with a strict manifest verified by `verify-report.py`.

## Catalog

| Report | Revision | Status | Experiment |
|---|---:|---|---|
| [Interannual candidate experiment 001](interannual-candidate-exp-001-report.md) | 2 | ACCEPTED | [A5b](../work-packages/20260713-a5b-interannual-candidate-spike/package.md) |
| [A5d1b finite-path realization](a5d1b-finite-path-realization-report.md) | 1 | ACCEPTED | [A5d1b](../work-packages/20260714-a5d1b-finite-path-realization/package.md) |
| [A5e0 direct annual-state pilot](a5e0-direct-annual-state-pilot-report.md) | 1 | ACCEPTED | [A5e0](../work-packages/20260714-a5e0-direct-annual-state-pilot/package.md) |
| [A7a daily precipitation-structure baseline](a7a-daily-precipitation-structure-report.md) | 1 | ACCEPTED | [A7a](../work-packages/20260714-a7a-daily-precipitation-structure-baseline/package.md) |
