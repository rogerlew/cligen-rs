...
archive = os.path.join(root, 'streams.npz')
if not os.path.exists(archive):
    with open(archive, 'wb') as stream:
        stream.write(b'')
for seed in ('147031', '271828', '314159'):
    checkpoint = os.path.join(root, 'seed-work', seed, 'checkpoint.pt')
...
if os.path.exists(partial):
    ...
    value = {
        'classification': 'a10m5r12-development-only-continuous-temporal',
