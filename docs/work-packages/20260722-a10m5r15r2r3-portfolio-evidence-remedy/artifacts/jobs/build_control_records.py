...
PACKAGE_ID = "20260722-a10m5r15r2r3-portfolio-evidence-remedy"
RUN_ID = "a10m5r15r2r3-portfolio-evidence-remedy-r0"
RECORD_TYPE = "a10m5r15r2r3-submission-admission"
...
def successor_control_bundle(record_commit: str | None = None) -> dict:
    identity = json.loads(CONTROL_IDENTITY.read_text(encoding="utf-8"))
    ...
    if not (
        ...
        and identity.get("all_static_identities_exact") is True
        and identity.get("candidate_output_accessed") is False
        and identity.get("protected_roles_opened") == []
        and [row.get("actual") for row in identity.get("models", [])] == expected
        ...
        raise RuntimeError("successor control calibration evidence invalid")
    files = (
        CONTROL_CALIBRATION / "package.md",
        CONTROL_IDENTITY,
        CONTROL_COLLECTION,
        CONTROL_TERMINAL,
    ...
    if record_commit is not None:
        for path in files:
            if path.read_bytes() != git_bytes(record_commit, path):
                raise RuntimeError(f"successor control evidence differs from published source: {path.name}")
    return {
        "candidate_output_accessed": False,
        "collection_record_sha256": collection["record_sha256"],
        "identity_artifact_sha256": digest(CONTROL_IDENTITY),
        "reconstruction_contract_sha256": digest(SUCCESSOR_RECONSTRUCTION),
        "terminal_record_sha256": terminal["record_sha256"],
        "valid": True,
    }


def campaign_accounting_bundle(record_commit: str | None = None) -> dict:
    campaign.validate(record_commit)
    return {
        "artifact_sha256": digest(CAMPAIGN_ACCOUNTING),
        "authorized_outer_ceiling_l40_minutes": 597,
        "bounded_maximum_l40_minutes": 575,
        "realized_l40_minutes": 60,
        "valid": True,
    }


def rebind_evidence_roles(paths: list[str]) -> list[str]:
    """Move every inherited role-scoped evidence path onto the R2 role names."""
    rebound = []
    for path in paths:
        for old_role, (new_role, _, _) in zip(OLD_ROLES, ROLES):
            path = path.replace(f"/{old_role}/", f"/{new_role}/")
            path = path.replace(
                f"admissions/{old_role}.json", f"admissions/{new_role}.json"
            )
        rebound.append(path)
    return list(dict.fromkeys(rebound))


def authority(options) -> None:
    predecessor_bundle(options.source_commit)
    calibration_bundle(options.source_commit)
    successor_control_bundle(options.source_commit)
    campaign_accounting_bundle(options.source_commit)
    ...
    value.update(
        {
            ...
            "successor_control_calibration": successor_control_bundle(),
            "campaign_accounting": campaign_accounting_bundle(),
...
def plan(options) -> None:
    ...
    value["successor_control_calibration"] = successor_control_bundle()
    value["campaign_accounting"] = campaign_accounting_bundle()
    ...
    value["evidence_allowlist"] = list(dict.fromkeys([*value["evidence_allowlist"], *extra_evidence]))
