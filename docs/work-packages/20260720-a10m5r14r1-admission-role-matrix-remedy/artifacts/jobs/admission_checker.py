#!/usr/bin/python3.11
"""Fail-closed admission checker for staged A10M5R14R1 submissions."""
...
def check(options: argparse.Namespace) -> tuple[dict[str, Any], bool]:
    ...
    materialization = plan.get("admission_materialization")
    gates: dict[str, bool] = {
        "admission_materialization_protocol": (
            isinstance(materialization, dict)
            and materialization.get("asset") == "materialize_admission.py"
            and isinstance(materialization.get("receipt_directory"), str)
            and materialization["receipt_directory"].startswith("/")
            and materialization.get("record_type")
            == "a10m5r14r1-submission-admission"
            and materialization.get("required_before_each_submit") is True
            and materialization.get("required_roles") == all_roles
            and materialization.get("snapshot")
            == "exact private toolkit state plus authenticated job receipts"
            and materialization.get("toolkit_submit_invokes_package_checker")
            is False
        ),
        ...
        "staged_asset_manifest_plan_identity": (
            plan_assets.get("asset-manifest.json", {}).get("bytes")
            == options.asset_manifest.stat().st_size
            and plan_assets.get("asset-manifest.json", {}).get("sha256")
            == manifest_sha256
        ),
        ...
        "exact_role_matrix": (
            plan_roles == all_roles
            and len(set(plan_roles)) == len(all_roles)
            and len(all_roles) == 1 + contract["resources"]["candidate_role_count"]
        ),
    ...
    attempt_keys_valid = all(
        isinstance(key, str)
        and isinstance(attempt, dict)
        and attempt.get("attempt_index") == 0
        and attempt.get("job_role") in all_roles
        and key == f"{attempt.get('job_role')}.0"
        for key, attempt in attempts.items()
    )
    gates["attempt_key_identity"] = attempt_keys_valid

    if target == CONTROL_ROLE:
        gates.update(
            {
                "control_predecessor": (
                    contract["admission"].get("control_must_pass_before_candidates")
                    is True
                ),
                "first_role_setup_ready": True,
                "maximum_live_candidate_jobs": len(attempts) == 0,
                "maximum_simultaneous_bootstraps": (
                    len(attempts) == 0
                    and contract["admission"].get(
                        "maximum_simultaneous_bootstraps"
                    )
                    == 1
                ),
        ...
        control_valid = (
            ...
            and control.get("state") == "RESULT_VALIDATED"
        )
        control_hash = None
        if control_valid:
        ...
        same_wave_prior = waves[wave_index][:member_index]
        expected_candidate_roles = set(prior_roles) | set(same_wave_prior)
        actual_candidate_roles = {
            attempt.get("job_role")
            for attempt in attempts.values()
            if isinstance(attempt, dict) and attempt.get("job_role") in roles
        }
        gates["no_future_or_out_of_order_attempts"] = (
            actual_candidate_roles == expected_candidate_roles
        )

        setup_hashes: dict[str, str] = {}
        same_wave_receipt_hashes: dict[str, str] = {}
        expected_setup_roles: set[str] = set()
        prefix_valid = True
        live_count = 0
        for role in same_wave_prior:
            attempt = attempts.get(f"{role}.0")
            if not isinstance(attempt, dict):
                prefix_valid = False
                continue
            state_name = attempt.get("state")
            if state_name == "SUBMITTED":
                ...
                prefix_valid = prefix_valid and ready
                if setup_hash is not None:
                    setup_hashes[role] = setup_hash
            elif state_name == "RESULT_VALIDATED":
                terminal, receipt_hash = job_receipt(
                    options.publication_dir, role, attempt, require_pass=True
                )
                prefix_valid = prefix_valid and terminal
                if receipt_hash is not None:
                    same_wave_receipt_hashes[role] = receipt_hash
            else:
                # REGISTERED, RESERVED, TERMINAL_OBSERVED, and unknown states
                # are not admissible prefix evidence.
                prefix_valid = False

        gates["first_role_setup_ready"] = prefix_valid
        gates["same_wave_order"] = (
            actual_candidate_roles == expected_candidate_roles
        )
        gates["same_wave_terminal_cleanup"] = prefix_valid
        gates["setup_inputs_exact"] = set(setups) == expected_setup_roles
        input_hashes["ready_setup_receipts"] = setup_hashes
        input_hashes["same_wave_job_receipts"] = same_wave_receipt_hashes

        gates["maximum_live_candidate_jobs"] = (
            live_count + 1
            <= contract["admission"]["maximum_live_candidate_jobs"]
        )
        gates["maximum_simultaneous_bootstraps"] = (
            prefix_valid
            and contract["admission"].get("maximum_simultaneous_bootstraps")
            == 1
        )
