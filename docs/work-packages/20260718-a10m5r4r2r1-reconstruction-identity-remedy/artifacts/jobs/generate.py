#!/usr/bin/env python3
"""Verify one accepted semantic state and summarize its frozen streams."""
...
def require_equal(label: str, actual: Any, expected: Any) -> None:
    if actual != expected:
        raise RuntimeError(f"semantic reconstruction mismatch: {label}")
...
def verify_reconstruction(output: Path, expected: dict[str, Any]) -> dict[str, Any]:
    ...
    model_record_path = output / "model-record.json"
    export_metadata = read_json(output / "export-metadata.json")
    checks = {
        ...
        "model_record_sha256": digest(model_record_path),
        "capacity_id": export_metadata["capacity_id"],
        "family": export_metadata["family"],
        "hidden_size": export_metadata["hidden_size"],
        "parameter_count": export_metadata["parameter_count"],
        "validation_primary_nll": export_metadata["validation_primary_nll"],
        "validation_stability": export_metadata["validation_stability"],
        "validation_tail_score": export_metadata["validation_tail_score"],
    }
    for label, actual in checks.items():
        require_equal(label, actual, expected[label])
    require_equal("row_id", export_metadata["row_id"], expected["row_id"])
    require_equal("metadata model record", export_metadata["model_record_sha256"], expected["model_record_sha256"])
    return checks
...
def generate_stream(
    ...
    return output, hashlib.sha256(output.tobytes()).hexdigest()
...
def main() -> None:
    ...
    parser.add_argument("--reconstruction-contract", type=Path, required=True)
    parser.add_argument("--sites", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    options = parser.parse_args()
    contract = read_json(options.contract)
    reconstruction = read_json(options.reconstruction_contract)
    expected = next((value for value in reconstruction["models"] if value["row_id"] == options.row_id), None)
    if expected is None:
        raise RuntimeError("row is not registered in reconstruction contract")
    model = next((value for value in contract["models"] if value["row_id"] == options.row_id), None)
    if model is None:
        raise RuntimeError("row is not registered in temporal contract")
    semantic = verify_reconstruction(options.output, expected)
    ...
    sites = read_json(options.sites)["sites"]
    records = []
    for site in sites:
        for member in generation["member_ids"]:
            values, payload_sha = generate_stream(
                module,
    ...
    expected_count = len(sites) * len(generation["member_ids"])
    atomic_json(options.output / "streams.json", {
        "capacity_id": model["capacity_id"],
        "non_gating_export_bytes": export.stat().st_size,
        "non_gating_export_sha256": export_sha,
    ...
    atomic_json(options.output / "evidence.json.part", {
        "classification": "a10m5r4r2r1-development-only-realized-temporal-streams",
        "gates": {
            "all_streams_support": all(record["support"] for record in records),
            "confirmation_sealed": True,
            "development_selection_sealed": True,
            "expected_stream_count": len(records) == expected_count,
