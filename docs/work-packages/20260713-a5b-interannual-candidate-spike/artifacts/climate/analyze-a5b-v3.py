...
Usage:
  analyze-a5b-v3.py CANDIDATE_MANIFEST CANDIDATE_ARCHIVE_DIR \
      BASELINE_MANIFEST BASELINE_ARCHIVE OBSERVED_TARGET OUTPUT
  analyze-a5b-v3.py --self-test
...
V2_ANALYZER = ANALYZER_SOURCE.with_name("analyze-a5b-v2.py")
V2_INCOMPLETE_ANALYSIS = ANALYZER_SOURCE.with_name(
    "a5b-analysis-v2-incomplete.json.gz"
)
...
V9_WEPP_ANALYZER = PACKAGE / "artifacts/wepp/analyze-wepp-v9.py"
...
def embedded_target_identity_matches(
    candidate: ReportVector, baseline: ReportVector, binding: Binding
) -> bool:
    index = binding.report_index
    return candidate.targets[index] == baseline.targets[index] and (
        binding.target_scale_path is None
        or candidate.scales[index] == baseline.scales[index]
    )
    ...
    for station in stations:
        ...
        if gate_id == 3:
            for rep, (candidate_row, baseline_row) in enumerate(
                ...
                for binding in eligible:
                    if not embedded_target_identity_matches(
                        candidate_row, baseline_row, binding
                    ):
                        evidence_failures.append(
                            f"{candidate_id}/{station_id}/{horizon}/rep{rep}: "
...
def self_test() -> dict[str, Any]:
    ...
    embedded_binding = next(
        binding
        for binding in bindings
        if binding.gate_id == 3
        and binding.surface_id == "report_embedded_station_parameter"
        and binding.target_scale_path is None
    )
    embedded_targets = np.full(len(bindings), np.nan, dtype=np.float64)
    embedded_targets[embedded_binding.report_index] = 3.5
    embedded_scales = np.full(len(bindings), np.nan, dtype=np.float64)
    embedded_candidate = ReportVector(
        np.zeros(len(bindings)),
        [10] * len(bindings),
        embedded_targets.copy(),
        [7] * len(bindings),
        embedded_scales.copy(),
        "a" * 64,
    )
    embedded_baseline = ReportVector(
        np.zeros(len(bindings)),
        [10] * len(bindings),
        embedded_targets.copy(),
        [5] * len(bindings),
    ...
    if not embedded_target_identity_matches(
        embedded_candidate, embedded_baseline, embedded_binding
    ):
        raise AssertionError("different generated sample counts changed target identity")
    embedded_candidate.targets[embedded_binding.report_index] = 3.6
    if embedded_target_identity_matches(
        embedded_candidate, embedded_baseline, embedded_binding
    ):
        raise AssertionError("changed embedded target was accepted")
    ...
    return {
        ...
        "checks": [
            ...
            "embedded_target_identity_ignores_generated_sample_count",
