...
Usage:
  analyze-a5b.py CANDIDATE_MANIFEST CANDIDATE_ARCHIVE_DIR \
      BASELINE_MANIFEST BASELINE_ARCHIVE OBSERVED_TARGET OUTPUT
  analyze-a5b.py --self-test
...
def nearest_rank(values: Sequence[float], numerator: int, denominator: int) -> float:
    if not values or not 0 < numerator <= denominator:
...
def validate_analysis_freeze_value(
    ...
    analyzer_relative = ANALYZER_SOURCE.relative_to(ROOT).as_posix()
...
def validate_analysis_freeze(
    ...
    return validate_analysis_freeze_value(
        freeze,
...
def synthetic_analysis_freeze() -> dict[str, Any]:
    ...
    frozen[ANALYZER_SOURCE.relative_to(ROOT).as_posix()] = sha256_path(
        ANALYZER_SOURCE
    )
    return {
