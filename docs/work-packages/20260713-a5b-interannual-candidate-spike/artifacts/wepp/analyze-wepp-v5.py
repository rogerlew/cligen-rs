...
Production usage:
  analyze-wepp-v5.py CAMPAIGN_INDEX CAMPAIGN_ARCHIVE_DIR CLIMATE_ANALYSIS
...
CAMPAIGN_CONTRACT = Path(__file__).with_name("wepp-campaign-v2.md")
CAMPAIGN_RUNNER = Path(__file__).with_name("run-wepp-matrix-v5.py")
...
def verify_frozen_tooling(index: dict[str, Any], climate_path: Path) -> dict[str, Any]:
    ...
    if (
        post_freeze.get("post_climate_wepp_freeze_version") != 1
    ...
    post_inventories = (
        ...
        require_dict(post_freeze.get("v5_artifacts"), "post-freeze v5 artifacts"),
    )
...
def validate_element_fixed_width_overflow(
    ...
    """Validate same-day reduction, PeakRO recovery, and the Sm exception."""
    ...
    if any(
        ...
        event_records <= 0
        or not 1 <= event_keys <= event_records
    ...
    overflow = require_dict(
        parser_record["element_fixed_width_overflow"],
        f"run {sequence} element fixed-width overflow audit",
    )
    require_exact_keys(
        overflow,
        {
    ...
    if overflow["policy_id"] != ELEMENT_OVERFLOW_POLICY_ID:
        raise AnalysisError(f"run {sequence}: element overflow policy differs")
    if overflow["source_element_sha256"] != element_artifact.sha256:
        raise AnalysisError(f"run {sequence}: element overflow source hash differs")
    allowed = require_dict(
        overflow["allowed"], f"run {sequence} element overflow allowance"
    )
    require_exact_keys(
        allowed, set(ELEMENT_OVERFLOW_ALLOWED), f"run {sequence} element overflow allowance"
    )
    if allowed != ELEMENT_OVERFLOW_ALLOWED:
        raise AnalysisError(f"run {sequence}: element overflow allowance differs")

    total_rows = overflow["total_element_rows"]
    if (
        isinstance(total_rows, bool)
        or not isinstance(total_rows, int)
        or total_rows != element_rows
    ):
        raise AnalysisError(f"run {sequence}: element overflow row count differs")
    observed = require_dict(
        overflow["observed"], f"run {sequence} element overflow observation"
    )
    if not observed:
        return 0, duplicate_rows, peak_count
    require_exact_keys(
        observed,
        {"count", "first_mapped_key"},
        f"run {sequence} element overflow observation",
    )
    ...
    if (
        ...
        raise AnalysisError(f"run {sequence}: element overflow count is invalid")
    first_key = require_dict(
        observed["first_mapped_key"], f"run {sequence} first element overflow key"
    )
    require_exact_keys(
        first_key,
        {"simulation_year", "ordinal_day", "ofe"},
        f"run {sequence} first element overflow key",
    )
    ...
    if any(
        ...
        raise AnalysisError(f"run {sequence}: first element overflow key is not integral")
    if not 1 <= simulation_year <= horizon or ofe != 1:
        raise AnalysisError(f"run {sequence}: first element overflow key is outside the run")
    legacy_leap = simulation_year % 4 == 0 or (
        horizon == 100 and simulation_year == 100
    )
    if not 1 <= ordinal_day <= (366 if legacy_leap else 365):
        raise AnalysisError(f"run {sequence}: first element overflow ordinal is invalid")
    return count, duplicate_rows, peak_count
...
def validate_campaign_element_fixed_width_overflow_counts(
    ...
    derived_overflow_count = sum(
        record.element_fixed_width_overflow_count for record in records
    )
    ...
    recorded_overflow_count = campaign_overflow_counts["Sm"]
    if (
        isinstance(recorded_overflow_count, bool)
        or not isinstance(recorded_overflow_count, int)
        or recorded_overflow_count < 0
        or recorded_overflow_count != derived_overflow_count
    ):
        raise AnalysisError(
            "campaign element fixed-width overflow aggregate differs from run audits"
        )
    return derived_overflow_count
...
def synthetic_campaign(directory: Path, semantic: Any) -> tuple[Path, dict[str, Any]]:
    ...
    index = {
        ...
        "execution": {
            ...
            "element_fixed_width_overflow_counts": {"Sm": 11},
...
def self_test() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="a5b-wepp-analysis-selftest-", dir=target) as raw:
        ...
        if (
            ...
            or index["execution"]["element_fixed_width_overflow_counts"] != {"Sm": 11}
        ):
