...
Production usage:
  analyze-wepp.py CAMPAIGN_INDEX CAMPAIGN_ARCHIVE_DIR CLIMATE_ANALYSIS
...
CAMPAIGN_CONTRACT = Path(__file__).with_name("wepp-campaign-v1.md")
CAMPAIGN_RUNNER = Path(__file__).with_name("run-wepp-matrix.py")
...
def verify_frozen_tooling(index: dict[str, Any], climate_path: Path) -> dict[str, Any]:
    ...
    for name, path in expected_contracts.items():
        ...
        if (
            ...
            or record["sha256"] != frozen_hash(path)
        ):
    ...
    return {
        ...
        "analyzer_sha256": frozen_hash(ANALYZER_SOURCE),
    }
