...
Production usage:
  run-wepp-matrix-v7.py CANDIDATE_MANIFEST CANDIDATE_CLI_DIR \
      CLIGEN_BINARY OUTPUT_DIR [--wepp-repo PATH] [--workers 1..4]

Self-test usage:
  run-wepp-matrix-v7.py --self-test [--wepp-repo PATH]
...
CAMPAIGN = RUNNER.with_name("wepp-campaign-v4.md")
ANALYZER = RUNNER.with_name("analyze-wepp-v7.py")
V4_CAMPAIGN = RUNNER.with_name("wepp-campaign-v1.md")
V4_RUNNER = RUNNER.with_name("run-wepp-matrix.py")
V4_ANALYZER = RUNNER.with_name("analyze-wepp.py")
V5_CAMPAIGN = RUNNER.with_name("wepp-campaign-v2.md")
V5_RUNNER = RUNNER.with_name("run-wepp-matrix-v5.py")
V5_ANALYZER = RUNNER.with_name("analyze-wepp-v5.py")
...
V6_CAMPAIGN = RUNNER.with_name("wepp-campaign-v3.md")
V6_RUNNER = RUNNER.with_name("run-wepp-matrix-v6.py")
V6_ANALYZER = RUNNER.with_name("analyze-wepp-v6.py")
V6_AMENDMENT = PACKAGE / "artifacts/freeze/post-climate-wepp-amendment-v2.md"
V6_FREEZE = PACKAGE / "artifacts/freeze/post-climate-wepp-freeze-v2.json"
POST_CLIMATE_AMENDMENT = (
    PACKAGE / "artifacts/freeze/post-climate-wepp-amendment-v3.md"
)
POST_CLIMATE_FREEZE = (
...
SELF_TEST_PUBLICATION_SHA256 = "2bc13588488a876fb65a39d279f8f6cc382184b3fb73026cafbb5eaddf459574"
...
EXTRACTION_ADAPTER_ID = "a5b_wepp_p326_response_extractor_v7"
...
ELEMENT_EFFINT_NUMERIC_INDEX = 2
ELEMENT_EFFINT_HEADER_FIELD = "EffInt"
...
# When the pinned WEPP executable produces no runoff events, it omits the
# seven-line hillslope/event-section suffix above and writes the annual-summary
# marker immediately after this common prefix.  That branch is admitted only
# at this exact seam and is closed against the companion element output below.
EVENT_ZERO_RECORD_PREAMBLE_LINES = len(EVENT_PREAMBLE_TEMPLATE) - 7
...
class ParsedElement:
    ...
    effint_fixed_width_overflow_count: int
    effint_fixed_width_overflow_first_key: tuple[int, int, int] | None
...
def verify_normative_hashes() -> None:
    ...
    freeze = require_dict(strict_json(POST_CLIMATE_FREEZE), "post-climate WEPP freeze")
    if set(freeze) != {
        "post_climate_wepp_freeze_version",
        "status",
        "created_utc",
        "boundary",
        "immutable_anchors",
        "v7_artifacts",
        "mutable_input_anchor",
        "failed_campaign",
    }:
        raise CampaignError("post-climate WEPP freeze is not closed")
    if (
        freeze.get("post_climate_wepp_freeze_version") != 3
        or freeze.get("status") != "passed"
        or freeze.get("boundary")
        != {
        ...
        raise CampaignError("post-climate WEPP freeze status/boundary differs")
    ...
    for inventory_name, paths in expected_static.items():
        inventory = require_dict(freeze.get(inventory_name), inventory_name)
        expected_paths = {path.relative_to(ROOT).as_posix() for path in paths}
        if set(inventory) != expected_paths:
            raise CampaignError(f"post-climate {inventory_name} paths differ")
        for path in paths:
            relative = path.relative_to(ROOT).as_posix()
            record = require_dict(inventory.get(relative), f"{inventory_name}.{relative}")
            if set(record) != {"sha256", "bytes"}:
                raise CampaignError(f"post-climate artifact record is not closed: {relative}")
            assert_artifact(
                path,
                require_sha256(record.get("sha256"), f"{relative}.sha256"),
                require_integer(record.get("bytes"), f"{relative}.bytes"),
                f"post-climate frozen artifact {relative}",
            )
    mutable = require_dict(
        freeze.get("mutable_input_anchor"), "post-climate mutable input anchor"
    )
    if set(mutable) != {
        "path",
        "pre_wepp_sha256",
        ...
        raise CampaignError("post-climate mutable input anchor is not closed")
    if (
        mutable.get("path") != EXPECTED_CANDIDATE_MANIFEST.relative_to(ROOT).as_posix()
        or require_sha256(mutable.get("pre_wepp_sha256"), "mutable anchor SHA-256")
        != "b52d20b6e472995491ae3d81433f54a709a2a53c65c8b4753957c0ecb0193b50"
        or require_integer(mutable.get("pre_wepp_bytes"), "mutable anchor bytes")
        != 2_822_764
        or mutable.get("candidate_cli_bytes_removed_after_wepp") is not False
    ):
        raise CampaignError("post-climate mutable input anchor differs")
    failed = require_dict(freeze.get("failed_campaign"), "failed v6 campaign")
    if failed != {
        "runner_version": 6,
        "status": "failed_closed_before_publication",
        "private_validated_runs": 2_176,
        "publication_tree_removed": True,
        "new_candidate_response_values_inspected": False,
        "coordinate": {
            "station_id": "ca042319",
        ...
        "failure": (
            "runs/a5b-wepp-ca042319-faithful_off-30yr-rep3/execution.json: "
            "element PeakRO recovery: PeakRO recovery counts differ"
        ),
        "cause": "final_archive_revalidator_rejected_closed_zero_event_counts",
    }:
        raise CampaignError("post-climate failed-campaign disposition differs")
...
def parse_event_hydrology(path: Path, horizon: int) -> ParsedEventHydrology:
    ...
    if len(lines) < EVENT_ZERO_RECORD_PREAMBLE_LINES + 1:
    ...
    zero_record_section = annual_index == EVENT_ZERO_RECORD_PREAMBLE_LINES
    expected_preamble = (
        EVENT_PREAMBLE_TEMPLATE[:EVENT_ZERO_RECORD_PREAMBLE_LINES]
        if zero_record_section
        else EVENT_PREAMBLE_TEMPLATE
    )
    for index, expected in enumerate(expected_preamble):
    ...
    if not zero_record_section and annual_index <= len(EVENT_PREAMBLE_TEMPLATE):
        raise CampaignError(f"{path}: nonempty event hydrology section is truncated")
    peaks: dict[tuple[int, int, int], float] = {}
    peak_thousandths: dict[tuple[int, int, int], int] = {}
    record_count = 0
    duplicate_rows = 0
    ...
    if record_count - len(peaks) != duplicate_rows:
        raise CampaignError(f"{path}: event hydrology record counts do not close")
    if record_count == 0:
        if not zero_record_section:
            raise CampaignError(f"{path}: empty event hydrology section has the wrong seam")
    else:
        if zero_record_section:
            raise CampaignError(f"{path}: event block follows a zero-record seam")
        first_block = next(
...
def element_effint_token_is_overflow(token: str, label: str) -> bool:
    """Validate source F7.3 EffInt or identify its exact nonresponse overflow."""

    if token == ELEMENT_SM_OVERFLOW_TOKEN:
        return True
    if len(token) > 7 or ELEMENT_PEAK_NUMERIC_TOKEN.fullmatch(token) is None:
        raise CampaignError(
            f"{label}: invalid {ELEMENT_EFFINT_HEADER_FIELD} "
            f"{ELEMENT_SM_FORTRAN_FORMAT} field {token!r}"
        )
    return False
...
def element_effint_overflow_audit(
    element: ParsedElement, source_element_sha256: str
) -> dict[str, Any]:
    count = element.effint_fixed_width_overflow_count
    first_key = element.effint_fixed_width_overflow_first_key
    if count < 0 or (count == 0) != (first_key is None):
        raise CampaignError("inconsistent parsed EffInt fixed-width overflow state")
    observed: dict[str, Any] = {}
    if first_key is not None:
        year, ordinal, ofe = first_key
        observed = {
    ...
    return {
        ...
        "source_element_sha256": source_element_sha256,
        "allowed": {
            "field": ELEMENT_EFFINT_HEADER_FIELD,
            "numeric_index_after_key": ELEMENT_EFFINT_NUMERIC_INDEX,
            "token": ELEMENT_SM_OVERFLOW_TOKEN,
            "fortran_format": ELEMENT_SM_FORTRAN_FORMAT,
            "response_status": "not_consumed_by_a5b_response",
        },
...
def validate_element_effint_overflow_audit(
    value: Any, label: str, expected_source_sha256: str
) -> int:
    audit = require_dict(value, label)
    if set(audit) != {
        ...
        raise CampaignError(f"{label}: EffInt overflow audit is not closed")
    expected_allowed = {
        "field": ELEMENT_EFFINT_HEADER_FIELD,
        "numeric_index_after_key": ELEMENT_EFFINT_NUMERIC_INDEX,
        "token": ELEMENT_SM_OVERFLOW_TOKEN,
    ...
    if (
        audit.get("policy_id") != ELEMENT_EFFINT_OVERFLOW_POLICY_ID
        or require_sha256(audit.get("source_element_sha256"), f"{label}.source")
        != expected_source_sha256
        or audit.get("allowed") != expected_allowed
    ):
        raise CampaignError(f"{label}: EffInt overflow policy/source differs")
    total = require_integer(audit.get("total_element_rows"), f"{label}.total")
    if total <= 0:
        raise CampaignError(f"{label}: total element rows must be positive")
    observed = require_dict(audit.get("observed"), f"{label}.observed")
    if not observed:
        return 0
    if set(observed) != {"count", "first_mapped_key"}:
        raise CampaignError(f"{label}: EffInt observation is not closed")
    count = require_integer(observed.get("count"), f"{label}.observed.count")
    key = require_dict(observed.get("first_mapped_key"), f"{label}.first_key")
    if not 1 <= count <= total or set(key) != {
        "simulation_year", "ordinal_day", "ofe",
    }:
        raise CampaignError(f"{label}: EffInt observation is invalid")
    year = require_integer(key.get("simulation_year"), f"{label}.year")
    ordinal = require_integer(key.get("ordinal_day"), f"{label}.ordinal")
    ofe = require_integer(key.get("ofe"), f"{label}.ofe")
    if year <= 0 or not 1 <= ordinal <= 366 or ofe != 1:
        raise CampaignError(f"{label}: EffInt first key is invalid")
    return count
...
def validate_element_peak_recovery_audit(
    ...
    if (
        ...
        or records < 0
        or not 0 <= keys <= min(records, expected_element_keys)
...
def parse_element(
    ...
    effint_overflow_count = 0
    effint_overflow_first_key: tuple[int, int, int] | None = None
    ...
    with path.open("r", encoding="ascii", errors="strict", newline="") as handle:
        for line_number, line in enumerate(handle, 1):
            ...
            effint_overflow = element_effint_token_is_overflow(
                value_tokens[ELEMENT_EFFINT_NUMERIC_INDEX], f"{path}:{line_number}"
            )
            ...
            values = {
                ...
                and not (index == ELEMENT_EFFINT_NUMERIC_INDEX and effint_overflow)
            ...
            if effint_overflow:
                effint_overflow_count += 1
                if effint_overflow_first_key is None:
                    effint_overflow_first_key = key
    ...
    if event_hydrology.record_count == 0 and any(
        row["runoff"] != 0.0 or row["peak_milli"] != 0
        for row in rows.values()
    ):
        raise CampaignError(
            f"{path}: zero-record event output has nonzero companion Runoff/PeakRO"
        )
    return ParsedElement(
        ...
        effint_overflow_count,
        effint_overflow_first_key,
        peak_recovery_count,
...
def compose_execution_record(
    ...
    return {
        ...
        "parser": {
            ...
            "element_effint_fixed_width_overflow": element_effint_overflow_audit(
                element, element_artifact.sha256
            ),
...
def load_baseline_inputs() -> tuple[dict[tuple[str, int, int], ClimateIdentity], dict[str, bytes], dict[str, Any]]:
    ...
    for path in (V4_CAMPAIGN, V4_RUNNER, V4_ANALYZER, BASELINE_VERIFIER):
        relative = path.relative_to(ROOT).as_posix()
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix=f"{job.record_id}-", dir=staging_root) as raw_tmp:
            ...
            return RunResult(
                ...
                effint_fixed_width_overflow_count=(
                    element.effint_fixed_width_overflow_count
                ),
...
def validate_staged_campaign(
    ...
    if set(overflow_counts) != {
        ELEMENT_EFFINT_HEADER_FIELD,
        ELEMENT_SM_HEADER_FIELD,
    }:
        raise CampaignError("campaign element fixed-width overflow counts are not closed")
    expected_effint_overflow_count = require_integer(
        overflow_counts.get(ELEMENT_EFFINT_HEADER_FIELD),
        "campaign EffInt fixed-width overflow count",
    )
    ...
    if expected_effint_overflow_count < 0 or expected_sm_overflow_count < 0:
        raise CampaignError("campaign nonresponse fixed-width overflow count is negative")
    ...
    observed_effint_overflow_count = 0
        ...
        with tarfile.open(path, mode="r:gz") as archive:
            ...
            for member in members:
                ...
                if role == "response":
                    ...
                    observed_effint_overflow_count += (
                        validate_element_effint_overflow_audit(
                            parser_audit.get("element_effint_fixed_width_overflow"),
                            f"{member.name}: element EffInt fixed-width overflow",
                            element_sha256,
                        )
                    )
    ...
    if observed_effint_overflow_count != expected_effint_overflow_count:
        raise CampaignError(
            "campaign EffInt fixed-width overflow aggregate differs: "
            f"{observed_effint_overflow_count} != {expected_effint_overflow_count}"
        )
...
def execute_campaign(
    ...
    mutable_anchor = require_dict(
        post_freeze.get("mutable_input_anchor"), "post-climate mutable input anchor"
    )
    if (
        candidate_manifest_path != EXPECTED_CANDIDATE_MANIFEST.resolve(strict=True)
        or sha256_bytes(manifest_raw) != mutable_anchor.get("pre_wepp_sha256")
        or len(manifest_raw) != mutable_anchor.get("pre_wepp_bytes")
    ):
        raise CampaignError("candidate manifest differs from the post-climate input anchor")
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="a5b-wepp-build-", dir=ROOT / "target") as build_tmp:
            ...
            effint_fixed_width_overflow_count = sum(
                result.effint_fixed_width_overflow_count for result in results
            )
            ...
            index = {
                ...
                "execution": {
                    ...
                    "element_fixed_width_overflow_counts": {
                        ELEMENT_EFFINT_HEADER_FIELD: effint_fixed_width_overflow_count,
...
def synthetic_element(path: Path, horizon: int) -> None:
    ...
    for year in range(1, horizon + 1):
        ...
        tokens[ELEMENT_EFFINT_NUMERIC_INDEX] = (
            f"{values[ELEMENT_EFFINT_NUMERIC_INDEX]:.3f}"
        )
        ...
        if year == 1:
            ...
            duplicate_tokens[ELEMENT_EFFINT_NUMERIC_INDEX] = (
                f"{duplicate[ELEMENT_EFFINT_NUMERIC_INDEX]:.3f}"
            )
...
def self_test(wepp_repo: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="a5b-wepp-self-test-", dir=target_root) as temporary:
        ...
        if (
            ...
            or element.effint_fixed_width_overflow_count != 0
            or element.effint_fixed_width_overflow_first_key is not None
        ...
        zero_record_event_lines = [
            *source_event_lines[:EVENT_ZERO_RECORD_PREAMBLE_LINES],
            "     ANNUAL AVERAGE SUMMARIES",
        ]
        zero_record_event_path = write_event_mutation(
            "zero-record", zero_record_event_lines
        )
        zero_record_event = parse_event_hydrology(zero_record_event_path, 30)
        if (
            zero_record_event.record_count != 0
            or zero_record_event.peaks
            or zero_record_event.peak_thousandths
        ):
            raise CampaignError("self-test zero-record event parse differs")

        zero_element_lines = element_path.read_text(encoding="ascii").splitlines()
        for index in range(2, len(zero_element_lines)):
            fields = zero_element_lines[index].split()
            fields[4 + 1] = "0.000000"
            fields[4 + ELEMENT_PEAK_NUMERIC_INDEX] = "0.000"
            zero_element_lines[index] = " ".join(fields)
        zero_element_path = work / "synthetic-element-zero-record.dat"
        zero_element_path.write_text(
            "\n".join(zero_element_lines) + "\n", encoding="ascii"
        )
        zero_element = parse_element(zero_element_path, 30, zero_record_event)
        zero_element_artifact = artifact_path(zero_element_path)
        zero_event_artifact = artifact_path(zero_record_event_path)
        zero_peak_audit = element_peak_recovery_audit(
            zero_element,
            zero_element_artifact.sha256,
            zero_event_artifact.sha256,
        )
        if (
            validate_element_peak_recovery_audit(
                zero_peak_audit,
                "self-test zero-record PeakRO recovery",
                zero_element_artifact.sha256,
                zero_event_artifact.sha256,
                zero_element.record_count,
                len(zero_element.rows),
            )
            != 0
        ):
            raise CampaignError(
                "self-test zero-record PeakRO recovery audit differs"
            )

        invalid_zero_peak_audit = json.loads(json.dumps(zero_peak_audit))
        invalid_zero_peak_audit["crosschecked_unique_keys"] = 1
        try:
            validate_element_peak_recovery_audit(
                invalid_zero_peak_audit,
                "self-test invalid zero-record PeakRO recovery",
                zero_element_artifact.sha256,
                zero_event_artifact.sha256,
                zero_element.record_count,
                len(zero_element.rows),
            ...
            raise CampaignError(
                "self-test accepted inconsistent zero-record PeakRO recovery"
            )

        nonzero_zero_element_lines = list(zero_element_lines)
        nonzero_zero_fields = nonzero_zero_element_lines[2].split()
        nonzero_zero_fields[4 + ELEMENT_PEAK_NUMERIC_INDEX] = "0.001"
        nonzero_zero_element_lines[2] = " ".join(nonzero_zero_fields)
        nonzero_zero_element = work / "synthetic-element-false-zero-record.dat"
        nonzero_zero_element.write_text(
            "\n".join(nonzero_zero_element_lines) + "\n", encoding="ascii"
        )
        try:
            parse_element(nonzero_zero_element, 30, zero_record_event)
        except CampaignError:
            pass
        else:
            raise CampaignError(
                "self-test accepted zero-record event output with nonzero PeakRO"
            )

        empty_full_event_section = [
            *source_event_lines[:len(EVENT_PREAMBLE_TEMPLATE)],
            "     ANNUAL AVERAGE SUMMARIES",
        ]
        try:
            parse_event_hydrology(
                write_event_mutation("empty-full-section", empty_full_event_section),
                30,
            )
        except CampaignError:
            pass
        else:
            raise CampaignError("self-test accepted empty full event section")

        displaced_zero_record = [
            *source_event_lines[:EVENT_ZERO_RECORD_PREAMBLE_LINES],
            "",
            "     ANNUAL AVERAGE SUMMARIES",
        ]
        try:
            parse_event_hydrology(
                write_event_mutation("displaced-zero-record", displaced_zero_record),
                30,
            )
        except CampaignError:
            pass
        else:
            raise CampaignError("self-test accepted displaced zero-record seam")
        ...
        try:
            element_effint_token_is_overflow("000.200", "self-test zero-padded EffInt")
        except CampaignError:
            pass
        else:
            raise CampaignError("self-test accepted zero-padded element EffInt")
        ...
        overflow_effint_path = mutate_element_token(
            "overflow-effint", ELEMENT_EFFINT_NUMERIC_INDEX, ELEMENT_SM_OVERFLOW_TOKEN
        )
        overflow_effint = parse_element(
            overflow_effint_path, 30, event_hydrology
        )
        overflow_effint_artifact = artifact_path(overflow_effint_path)
        overflow_effint_audit = element_effint_overflow_audit(
            overflow_effint, overflow_effint_artifact.sha256
        )
        if (
            overflow_effint.effint_fixed_width_overflow_count != 1
            or overflow_effint.effint_fixed_width_overflow_first_key != (1, 1, 1)
            or validate_element_effint_overflow_audit(
                overflow_effint_audit,
                "self-test EffInt overflow",
                overflow_effint_artifact.sha256,
            )
            != 1
        ):
            raise CampaignError("self-test EffInt overflow closed audit differs")

        element_mutations = {
            "runoff-star": (1, ELEMENT_SM_OVERFLOW_TOKEN),
            "effint-wrong-star": (ELEMENT_EFFINT_NUMERIC_INDEX, "******"),
            "effint-overwidth": (ELEMENT_EFFINT_NUMERIC_INDEX, "1000.000"),
        ...
        if (
            ...
            != overflow_audit
            or self_test_execution["parser"][
                "element_effint_fixed_width_overflow"
            ]
            != element_effint_overflow_audit(element, element_artifact.sha256)
        ...
        publication_projection = {
            ...
            "element_fixed_width_overflow_counts": {
                ELEMENT_EFFINT_HEADER_FIELD: sum(
                    result.effint_fixed_width_overflow_count
