...
Production usage:
  analyze-wepp-v9.py CAMPAIGN_INDEX CAMPAIGN_ARCHIVE_DIR CLIMATE_ANALYSIS
...
CAMPAIGN_CONTRACT = Path(__file__).with_name("wepp-campaign-v4.md")
CAMPAIGN_RUNNER = Path(__file__).with_name("run-wepp-matrix-v7.py")
...
V6_CAMPAIGN_CONTRACT = Path(__file__).with_name("wepp-campaign-v3.md")
V6_CAMPAIGN_RUNNER = Path(__file__).with_name("run-wepp-matrix-v6.py")
V6_ANALYZER = Path(__file__).with_name("analyze-wepp-v6.py")
...
ORIGINAL_CLIMATE_ANALYZER = PACKAGE / "artifacts/climate/analyze-a5b.py"
V2_CLIMATE_ANALYZER = PACKAGE / "artifacts/climate/analyze-a5b-v2.py"
CLIMATE_ANALYZER = PACKAGE / "artifacts/climate/analyze-a5b-v3.py"
V2_INCOMPLETE_ANALYSIS = (
    PACKAGE / "artifacts/climate/a5b-analysis-v2-incomplete.json.gz"
)
...
V7_ANALYZER = ANALYZER_SOURCE.with_name("analyze-wepp-v7.py")
V8_ANALYZER = ANALYZER_SOURCE.with_name("analyze-wepp-v8.py")
V1_POST_OUTPUT_ANALYSIS_FREEZE = (
    PACKAGE / "artifacts/freeze/post-output-analysis-freeze-v1.json"
)
POST_OUTPUT_ANALYSIS_AMENDMENT = (
    PACKAGE / "artifacts/freeze/post-output-analysis-amendment-v2.md"
)
POST_OUTPUT_ANALYSIS_FREEZE = (
    PACKAGE / "artifacts/freeze/post-output-analysis-freeze-v2.json"
)
...
def verify_post_output_analysis_freeze() -> dict[str, str]:
    freeze = require_dict(
    ...
    if (
        freeze["post_output_analysis_freeze_version"] != 2
        or freeze["status"] != "passed"
        or freeze["boundary"]
        != {
            "candidate_climate_output_present": True,
            "candidate_metric_values_inspected": True,
            "wepp_response_output_present": True,
            "climate_analysis_output_present": True,
            "wepp_analysis_output_absent": True,
        }
    ):
    ...
    for name, paths in inventories.items():
        ...
        for path in paths:
            ...
            require_exact_keys(
                record, {"sha256", "bytes"}, f"post-output artifact {relative}"
            )
            artifact = require_artifact(record, f"post-output artifact {relative}")
            if (
                not path.is_file()
                or path.is_symlink()
            ...
            identities[relative] = artifact.sha256
    if freeze["failed_analysis"] != {
        "analyzer_version": 2,
        "status": "completed_with_incomplete_evidence",
        "candidate_metric_values_inspected": True,
        "climate_failure_count": 48_713,
        "cause": "generated_sample_count_compared_as_frozen_target_identity",
        "invalid_invariant": "candidate_target_count_equals_baseline_target_count",
        "retained_rules": [
            "station_parameter_identity",
            "embedded_target_value",
            "embedded_target_scale",
            "registered_count_sufficiency",
        ],
    }:
        raise AnalysisError("post-output failed-analysis disposition differs")
    identities[POST_OUTPUT_ANALYSIS_FREEZE.relative_to(ROOT).as_posix()] = (
        sha256_path(POST_OUTPUT_ANALYSIS_FREEZE)
    )
    return identities


def verify_frozen_tooling(index: dict[str, Any], climate_path: Path) -> dict[str, Any]:
    analysis_identities = verify_post_output_analysis_freeze()
    ...
    if (
        ...
        or post_freeze.get("post_climate_wepp_freeze_version") != 3
    ...
    post_inventories = (
        ...
        require_dict(post_freeze.get("v7_artifacts"), "post-freeze v7 artifacts"),
    )
    ...
    return {
        ...
        "analyzer_sha256": analysis_identities[
            ANALYZER_SOURCE.relative_to(ROOT).as_posix()
        ],
        "post_output_analysis_freeze_sha256": analysis_identities[
            POST_OUTPUT_ANALYSIS_FREEZE.relative_to(ROOT).as_posix()
        ],
    }
...
def execute(index_path: Path, archive_dir: Path, climate_path: Path) -> None:
    ...
    if (
        ...
        or climate_inputs.get("pre_candidate_analyzer_sha256")
        != sha256_path(ORIGINAL_CLIMATE_ANALYZER)
        or climate_inputs.get("post_output_analysis_freeze_sha256")
        != frozen_tooling["post_output_analysis_freeze_sha256"]
