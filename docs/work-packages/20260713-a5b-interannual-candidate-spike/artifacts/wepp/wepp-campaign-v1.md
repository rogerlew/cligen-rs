...
Adapter ID: `a5b_wepp_p326_response_extractor_v4`.
...
number except the independently recovered `PeakRO` case below. `Runoff` index
...
censor, or impute the response. This campaign requires at least one complete
event-hydrology block per successful run; a no-event stream is outside the
registered response surface and fails closed.

Each execution record carries three closed audits. The
...
`execution.element_fixed_width_overflow_counts = {"Sm": <count>}`, and
...
must name `a5b_wepp_p326_response_extractor_v4` and bind the frozen runner's
