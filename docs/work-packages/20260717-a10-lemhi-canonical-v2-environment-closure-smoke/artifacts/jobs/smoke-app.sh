...
test "$CUBLAS_WORKSPACE_CONFIG" = :4096:8
test "$PATH" = "$runtime_root/bin:$attempt_root/toolchain/bin:/usr/bin:/bin"
test "$TMPDIR" = "$attempt_root/tmp"
...
! /usr/bin/ldd "$runtime_root/bin/python3" | grep -q 'not found'
torch_extension=$(find "$environment/lib" -type f -path '*/torch/_C*.so' -print | head -n 1)
test -n "$torch_extension"
! /usr/bin/ldd "$torch_extension" | grep -q 'not found'
