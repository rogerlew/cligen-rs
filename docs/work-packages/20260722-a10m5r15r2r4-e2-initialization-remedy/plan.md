# A10M5R15R2R4 E2 Initialization-Remedy Plan

This corrective successor retains the R2R3 scientific and execution plan
unchanged. It only makes the already-required E2 descriptor initialization
write effective. The inherited evidence-role remedy remains intact. No
candidate role is released until the six-row successor-corpus reconstruction
contract reproduces.

1. Publish the corrected scaffold on `main`; do not prepare execution assets or
   request an allocation from an unpublished tree.
2. Authenticate and inherit the candidate-blind paired-attribution receipt
   from the closed R2 package, including bootstrap seeds 410542 and 410543.
   The receipt remains owned by that predecessor and is not regenerated or
   relabeled by R2R4.
3. Re-run the independent readiness review. Stop if any P1/P2 finding remains.
4. Build the exact R1 successor `corpus.tar`; verify its byte identity and all
   101 member-layout facts against the R2 corpus pin.
5. Prepare a fresh immutable asset directory from R14R2R2 r3. Replace the model
   and execution overlays, remove the stale inherited replay entry point, and
   publish the R2 post-collection replay source identity.
6. Generate fresh authority and a semantic plan from the exact final published
   commit. Authenticate the R1 predecessor, calibration receipt, corpus,
   calendar/missingness preflight, runtime, role map, and 515-minute ceiling.
7. Materialize and authenticate the control admission. Run the 30-minute
...
10. From the authenticated collected evidence, run
    ...
    terminal.
11. Close the toolkit run, verify remote and job-local cleanup, reconcile the
    package/catalog/roadmap/ExecPlan, run repository gates, independently review
    the final evidence, and commit and push the terminal record.

Candidate output is forbidden before stage 8. Stages 2 through 7 are hard
ordering gates, not advisory preparation.
