...
def main() -> None:
    ...
    parser.add_argument("--expected-identities", type=Path, required=True)
    ...
    expected_identities = json.loads(options.expected_identities.read_text(encoding="utf-8"))
    observed_identities = [
        {key: row[key] for key in ("station_id", "horizon_years", "candidate_identity")}
        for row in benchmark_result["rows"]
    ]
    candidate_identity_exact = observed_identities == expected_identities
