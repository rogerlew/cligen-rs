...
run_root=$PWD
output=$run_root/results/acceptance
...
if ! "$environment/bin/python" "$run_root/accept.py" \
  ...
  --output "$output" \
  --expected-identities "$run_root/expected-identities.json" 2>"$python_stderr"; then
  sed "s|$run_root|[REMOTE_RUN_ROOT]|g; s|$job_local|[JOB_LOCAL]|g" "$python_stderr" >&2 || true
  exit 1
fi
rm -f -- "$python_stderr"
printf '%s\n' "A10M5R1-ACCEPTANCE-COMPLETE $configuration_id"
