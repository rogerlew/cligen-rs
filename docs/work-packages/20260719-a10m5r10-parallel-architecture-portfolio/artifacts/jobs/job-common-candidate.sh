...
./supervise-v2.sh "$target" "$marker_sha" "$output/supervisor.json" \
  ./run_candidate.sh "$target" "$role" "$candidate" "$capacity"
status=$?
set -e
cleanup=false
...
/usr/bin/python3 - "$output/evidence.json.part" "$output/evidence.json" \
  ...
  "$role" "$run_id" "$uid" "$device" "$marker_sha" "$candidate" "$capacity" <<'PY'
import json, os, sys
partial, final, status, cleanup, target, job, node, role, run_id, uid, device, marker_sha, candidate, capacity = sys.argv[1:]
