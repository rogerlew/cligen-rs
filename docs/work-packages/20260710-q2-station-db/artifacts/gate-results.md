...
| `cargo test --release` | **103 passed, 0 failed** — includes the 12-golden byte-identity gates (untouched), the quality suite, and the new 9-test `stations` suite + 6 stations unit tests | 0 |
...
|---|---|---|
| `cargo publish --dry-run` clean under the size limit, no data in the tarball | `cargo publish --dry-run -p cligen --allow-dirty` — **76 files, 608.6 KiB (166.3 KiB compressed)** vs the 10 MB limit | exit 0 |
...
| Nearest matches a pinned independent oracle | Python (independent sqlite3 read + independent haversine) over 7 query points across all five collections, 5 rows each: **35/35 id+order matches, all distances within 1e-4 km**; plus the committed 7-station au oracle test (`nearest_matches_pinned_au_oracle`) | exit 0 |
...
| Redirect auth hygiene | `network_sync_follows_redirects_without_forwarding_auth` — loopback HTTP server observes the redirect hop carries no Authorization header | pass |
