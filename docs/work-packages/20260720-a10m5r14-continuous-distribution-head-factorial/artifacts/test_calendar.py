...
missing_index = dates.index(dt.date(1985, 6, 15))
missing["source_observed"][missing_index] = False
for field in ("prcp", "tmax", "tmin"):
    missing["fields"][field][missing_index] = None
try:
    module._long_windows(missing, 16)
except RuntimeError:
    pass
else:
    raise RuntimeError("non-structural missing day passed 16-year eligibility")

boundary = copy.deepcopy(document)
boundary_index = dates.index(dt.date(1996, 1, 1))
boundary["dates"].pop(boundary_index)
boundary["source_observed"].pop(boundary_index)
for field in ("prcp", "tmax", "tmin"):
    boundary["fields"][field].pop(boundary_index)
try:
    module._long_windows(boundary, 16)
except RuntimeError:
    pass
else:
    raise RuntimeError("missing 16-year stop boundary passed window eligibility")
