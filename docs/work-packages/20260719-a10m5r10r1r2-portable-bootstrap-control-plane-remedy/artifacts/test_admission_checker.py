#!/usr/bin/env python3
"""Self-tests for the staged A10M5R10R1R2 admission state machine."""
...
class AdmissionTests(unittest.TestCase):
    def setUp(self) -> None:
        self.temporary = tempfile.TemporaryDirectory(prefix="a10m5r10r1r2-admission-")
    ...
    def test_terminal_failed_first_member_stops_same_wave_admission(self) -> None:
        control = self.attempt(CHECKER.CONTROL_ROLE, "1001", "RESULT_VALIDATED", passed=True)
    ...
    def test_next_wave_requires_both_observed_cleanup_receipts(self) -> None:
        ...
        for index, role in enumerate(self.roles[:2], start=2):
            prior = self.attempt(role, str(1000 + index), "RESULT_VALIDATED", passed=True)
            attempts[f"{role}.0"] = prior
            self.publish_job(role, prior, passed=True)
        self.write_state("MATRIX_ACTIVE", attempts)
    ...
    def test_failed_prior_wave_stops_next_wave_admission(self) -> None:
        attempts: dict[str, object] = {}
        control = self.attempt(CHECKER.CONTROL_ROLE, "1001", "RESULT_VALIDATED", passed=True)
        attempts[f"{CHECKER.CONTROL_ROLE}.0"] = control
        self.publish_job(CHECKER.CONTROL_ROLE, control, passed=True)
        for index, role in enumerate(self.roles[:2], start=2):
            passed = index != 3
            prior = self.attempt(
                role, str(1000 + index), "RESULT_VALIDATED", passed=passed
            )
            attempts[f"{role}.0"] = prior
            self.publish_job(role, prior, passed=passed)
        self.write_state("MATRIX_ACTIVE", attempts)
        receipt, passed = CHECKER.check(self.options(self.roles[2]))
        self.assertFalse(passed)
        self.assertFalse(receipt["gates"]["no_observed_candidate_failure"])
