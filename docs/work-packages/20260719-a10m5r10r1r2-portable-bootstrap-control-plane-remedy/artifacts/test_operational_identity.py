...
class SetupIdentityTests(unittest.TestCase):
    def setUp(self) -> None:
        self.temporary = tempfile.TemporaryDirectory(prefix="a10m5r10r1r2-setup-")
        ...
        admission = {
            ...
            "record_type": "a10m5r10r1r2-submission-admission",
            "role": "test-role",
...
class PortableBootstrapTests(unittest.TestCase):
    ...
    def test_missing_runtime_publishes_minimal_failed_gate(self) -> None:
        with tempfile.TemporaryDirectory(prefix="a10m5r10r1r2-missing-runtime-") as temporary:
            root = Path(temporary)
    ...
    def test_oversized_pre_python_failure_log_is_bounded(self) -> None:
        with tempfile.TemporaryDirectory(prefix="a10m5r10r1r2-large-log-") as temporary:
            root = Path(temporary)
            run_root = root / BUILDER.RUN_ID
            run_root.mkdir()
...
class PredecessorBindingTests(unittest.TestCase):
    ...
    def test_authority_and_plan_preserve_predecessor_and_admissions(self) -> None:
        with tempfile.TemporaryDirectory(prefix="a10m5r10r1r2-builder-") as temporary:
            root = Path(temporary)
            source_commit = self.source_commit()
