...
class SetupIdentityTests(unittest.TestCase):
    def setUp(self) -> None:
        self.temporary = tempfile.TemporaryDirectory(prefix="a10m5r10r1r1-setup-")
...
class PredecessorBindingTests(unittest.TestCase):
    ...
    def test_r1_hold_and_toolkit_hardening_are_bound(self) -> None:
    ...
    def test_authority_and_plan_preserve_predecessor_and_admissions(self) -> None:
        with tempfile.TemporaryDirectory(prefix="a10m5r10r1r1-builder-") as temporary:
            root = Path(temporary)
            source_commit = self.source_commit()
