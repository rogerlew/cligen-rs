#!/usr/bin/python3.11
"""Publish durable, redacted setup diagnostics outside job-local storage."""
...
def redacted(value: str, run_root: Path, job_local: Path) -> str:
    return value.replace(str(run_root), "[REMOTE_RUN_ROOT]").replace(
        str(job_local), "[JOB_LOCAL]"
    )
...
def record(options: argparse.Namespace) -> None:
    ...
    admission_authenticated = (
        ...
        and admission.get("record_type") == "a10m5r10r1r1-submission-admission"
    ...
    execution_identity = {
        ...
        "host_python_path": options.host_python_path,
    ...
    execution_identity_authenticated = (
        options.run_id
        == "a10m5r10r1r1-python311-control-plane-remedy-r0"
        ...
        and options.host_python_path == "/usr/bin/python3.11"
    ...
    control_plane_python_authenticated = (
    ...
    valid = (
        ...
        and control_plane_python_authenticated
    ...
    value = {
        "authentication": {
            ...
            "control_plane_python_authenticated": control_plane_python_authenticated,
