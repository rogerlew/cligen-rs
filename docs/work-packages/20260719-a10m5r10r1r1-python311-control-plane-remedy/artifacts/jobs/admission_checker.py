#!/usr/bin/python3.11
"""Fail-closed admission checker for staged A10M5R10R1R1 submissions."""
...
def setup_ready(
    ...
    *,
    ...
    ready = (
        ...
        and execution.get("host_python_path") == "/usr/bin/python3.11"
...
def check(options: argparse.Namespace) -> tuple[dict[str, Any], bool]:
        ...
        for role in prior_roles:
            ...
            if valid:
                valid, receipt_hash = job_receipt(
                    options.publication_dir, role, attempt, require_pass=False
                )
        ...
        if member_index == 0:
            ...
            if isinstance(first_attempt, dict) and first_attempt.get("state") == "RESULT_VALIDATED":
                first_terminal, first_receipt_hash = job_receipt(
                    options.publication_dir, first_role, first_attempt, require_pass=False
                )
    ...
    receipt: dict[str, Any] = {
        ...
        "record_type": "a10m5r10r1r1-submission-admission",
        "role": target,
        "run_id": RUN_ID,
        "schema_version": 1,
        "source_commit": source_commit,
