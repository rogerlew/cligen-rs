...
record_setup() {
  ...
  /usr/bin/python3.11 "$run_root/setup_diagnostics.py" record \
...
run_logged() {
  ...
  step_status=$?
  set -e
  /usr/bin/python3.11 "$run_root/setup_diagnostics.py" log \
...
if run_logged host-python-version "$host_python" --version; then
  host_python_version=$("$host_python" --version 2>&1 || true)
  case "$host_python_version" in
    "Python 3.11."*) host_python_version_exit=0 ;;
    *) host_python_version_exit=65 ;;
  esac
else
  host_python_version_exit=$?
fi
record_setup initialized false
if [ "$host_python_version_exit" -ne 0 ]; then
  exit "$host_python_version_exit"
fi
...
printf '%s\n' A10M5R10R1R1-ENVIRONMENT-READY
