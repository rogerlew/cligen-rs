...
target=${TMPDIR:-/tmp}/a10m5r10r1r1-$role-$SLURM_JOB_ID
...
./supervise-v2.sh "$target" "$marker_sha" "$output/supervisor.json" \
  ./run_control.sh "$target" "$role" "$run_id" "$SLURM_JOB_ID" \
...
/usr/bin/python3.11 - "$output" "$status" <<'PY'
import json, os, sys
...
PY
/usr/bin/python3.11 - "$output/evidence.json.part" "$output/evidence.json" \
...
partial, final, setup_path, admission_path, manifest_path, status, cleanup, target, job, node, role, run_id, uid, device, marker_sha = sys.argv[1:]
def canonical(value):
