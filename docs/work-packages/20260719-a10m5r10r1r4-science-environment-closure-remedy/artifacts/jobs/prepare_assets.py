#!/usr/bin/env python3
"""Prepare immutable assets for the staged A10M5R10R1R4 portfolio rerun."""
...
PREDECESSOR_IDENTITY_MANIFEST_BYTES = 7720
...
def verify_corpus_archive(path: Path) -> None:
    if identity(CORPUS_LAYOUT_PIN) != {
        "bytes": CORPUS_LAYOUT_PIN_BYTES,
        "sha256": CORPUS_LAYOUT_PIN_SHA256,
    }:
    ...
    if (
        pin.get("package_id")
        != "20260719-a10m5r10r1r4-science-environment-closure-remedy"
        or verifier.identity(path) != pin.get("archive")
        or verifier.inspect_layout(path) != pin.get("layout")
    ):
        raise RuntimeError("canonical corpus archive identity/layout drift")
...
def main() -> None:
    ...
    verify_corpus_archive(root / "corpus.tar")
