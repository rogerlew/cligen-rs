#!/usr/bin/env python3
"""Build private A10M5R10R1R4 authority input and staged portfolio plan."""
...
AUTHORITY_ID = "a10m5r10r1r4-science-environment-closure-remedy-authority"
BUDGET_ID = "a10m5r10r1r4-science-environment-closure-remedy-budget"
AUTHORITY_TOKEN = "a10m5r10r1r4-science-environment-closure-remedy-authority-token"
...
CORPUS_LAYOUT_PIN = PACKAGE / "artifacts/corpus-layout-pin.json"
CORPUS_LAYOUT_PIN_BYTES = 864
CORPUS_LAYOUT_PIN_SHA256 = (
...
def verify_corpus_asset_root(asset_root: Path) -> None:
    if (
        not CORPUS_LAYOUT_PIN.is_file()
        or CORPUS_LAYOUT_PIN.is_symlink()
        or CORPUS_LAYOUT_PIN.stat().st_size != CORPUS_LAYOUT_PIN_BYTES
        or digest(CORPUS_LAYOUT_PIN) != CORPUS_LAYOUT_PIN_SHA256
    ):
        raise RuntimeError("corpus layout pin byte identity drift")
    specification = importlib.util.spec_from_file_location(
        "verify_corpus_layout", PACKAGE / "artifacts/verify_corpus_layout.py"
    )
    if specification is None or specification.loader is None:
        raise RuntimeError("cannot load corpus layout verifier")
    verifier = importlib.util.module_from_spec(specification)
    specification.loader.exec_module(verifier)
    pin = verifier.load_pin(CORPUS_LAYOUT_PIN)
    archive = asset_root / "corpus.tar"
    manifest = json.loads(
        (asset_root / "asset-manifest.json").read_text(encoding="utf-8")
    )
    if (
        verifier.identity(archive) != pin["archive"]
        or verifier.inspect_layout(archive) != pin["layout"]
        or manifest.get("assets", {}).get("corpus.tar") != pin["archive"]
        or manifest.get("package_id") != PACKAGE_ID
    ):
        raise RuntimeError("pre-authority corpus identity/layout preflight failed")
...
def predecessor_bundle() -> dict[str, object]:
    ...
    for predecessor_name, predecessor in document["predecessors"].items():
        record_commit = predecessor.get("record_commit")
        if record_commit is not None:
            ...
            if ancestry.returncode != 0:
                raise RuntimeError(
                    f"predecessor record commit is not an ancestor: {predecessor_name}"
                )
        ...
        for name, expected in predecessor["files"].items():
            ...
            if record_commit is not None:
                committed = subprocess.run(
                    ("git", "show", f"{record_commit}:{expected['source_path']}"),
                    cwd=REPO,
                    check=True,
                    capture_output=True,
                ).stdout
                if (
                    len(committed) != expected["bytes"]
                    or hashlib.sha256(committed).hexdigest() != expected["sha256"]
                ):
                    raise RuntimeError(
                        f"committed predecessor identity mismatch: {predecessor_name}/{name}"
                    )
    ...
    r1r3 = bound["a10m5r10r1r3_cublas_environment_scope_hold"]
    r1r3_summary_path = Path(
        r1r3["files"]["operational-summary.json"]["local_path"]
    )
    r1r3_summary = json.loads(r1r3_summary_path.read_text(encoding="utf-8"))
    r1r3_cleanup_path = Path(r1r3["files"]["cleanup-record.json"]["local_path"])
    r1r3_cleanup = json.loads(r1r3_cleanup_path.read_text(encoding="utf-8"))
    if (
        r1r3_summary.get("package_id") != r1r3["package_id"]
        or r1r3_summary.get("run_id") != r1r3["run_id"]
        or r1r3_summary.get("terminal") != r1r3["terminal"]
        or r1r3_summary.get("root_cause", {}).get("classification")
        != "cublas-environment-scope"
        or r1r3_summary.get("root_cause", {}).get("required_assignment")
        != "CUBLAS_WORKSPACE_CONFIG=:4096:8"
        or r1r3_summary.get("root_cause", {}).get("scheduler_export_policy")
        != "NONE"
        or r1r3_summary.get("root_cause", {}).get("wrapper_parent_exported")
        is not False
        or r1r3_summary.get("execution", {}).get("control_rows_published", 0)
        != 0
        or r1r3_summary.get("execution", {}).get("candidate_jobs_submitted") != 0
        or r1r3_summary.get("science", {}).get(
            "architecture_interpretation_authorized"
        )
        is not False
        or r1r3_cleanup.get("remote_absent") is not True
        or r1r3_cleanup.get("package_id") != r1r3["package_id"]
        or r1r3_cleanup.get("run_id") != r1r3["run_id"]
    ):
        raise RuntimeError("R1R3 CuBLAS environment scope HOLD semantic drift")

    r1r2 = bound["a10m5r10r1r2_corpus_root_hold"]
    r1r2_summary_path = Path(
        r1r2["files"]["operational-summary.json"]["local_path"]
    )
    r1r2_summary = json.loads(r1r2_summary_path.read_text(encoding="utf-8"))
    r1r2_cleanup_path = Path(r1r2["files"]["cleanup-record.json"]["local_path"])
    r1r2_cleanup = json.loads(r1r2_cleanup_path.read_text(encoding="utf-8"))
    if (
        r1r2_summary.get("package_id") != r1r2["package_id"]
        or r1r2_summary.get("run_id") != r1r2["run_id"]
        or r1r2_summary.get("terminal") != r1r2["terminal"]
        or r1r2_summary.get("root_cause", {}).get("classification")
        != "corpus-root-nesting"
        or r1r2_summary.get("root_cause", {}).get("archive_sha256")
        != "8770e127f8413eedd47d50670c359e450988444a8c4d8d43ca5645619a1b0a17"
        or r1r2_summary.get("science", {}).get(
            "architecture_interpretation_authorized"
        )
        is not False
        or r1r2_cleanup.get("remote_absent") is not True
        or r1r2_cleanup.get("package_id") != r1r2["package_id"]
        or r1r2_cleanup.get("run_id") != r1r2["run_id"]
    ):
        raise RuntimeError("R1R2 corpus root HOLD semantic drift")

    r1r1 = bound["a10m5r10r1r1_compute_python_hold"]
    r1r1_summary_path = Path(
        r1r1["files"]["operational-summary.json"]["local_path"]
    )
    r1r1_summary = json.loads(r1r1_summary_path.read_text(encoding="utf-8"))
    r1r1_cleanup_path = Path(r1r1["files"]["cleanup-record.json"]["local_path"])
    r1r1_cleanup = json.loads(r1r1_cleanup_path.read_text(encoding="utf-8"))
    if (
        r1r1_summary.get("package_id") != r1r1["package_id"]
        or r1r1_summary.get("run_id") != r1r1["run_id"]
        or r1r1_summary.get("terminal") != r1r1["terminal"]
        or r1r1_summary.get("root_cause", {}).get("classification")
        != "compute-python311-binary-absent"
        or r1r1_summary.get("root_cause", {}).get("runtime_archive_opened")
        is not False
        or r1r1_summary.get("science", {}).get(
            "architecture_interpretation_authorized"
        )
        is not False
        or r1r1_cleanup.get("remote_absent") is not True
        or r1r1_cleanup.get("package_id") != r1r1["package_id"]
        or r1r1_cleanup.get("run_id") != r1r1["run_id"]
    ):
        raise RuntimeError("R1R1 compute Python HOLD semantic drift")
...
def authority(options: argparse.Namespace) -> None:
    verify_corpus_asset_root(options.asset_root)
    contract = json.loads(
...
def plan(options: argparse.Namespace) -> None:
    ...
    value = {
        ...
        "required_job_environment": dict(REQUIRED_JOB_ENVIRONMENT),
        "resource_budget_id": BUDGET_ID,
        "run_id": RUN_ID,
        "scheduler_authority_token": AUTHORITY_TOKEN,
        "source_commit": options.source_commit,
