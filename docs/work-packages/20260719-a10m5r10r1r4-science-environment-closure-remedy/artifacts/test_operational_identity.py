...
class SetupIdentityTests(unittest.TestCase):
    def setUp(self) -> None:
        self.temporary = tempfile.TemporaryDirectory(prefix="a10m5r10r1r4-setup-")
...
class PortableBootstrapTests(unittest.TestCase):
    ...
    def test_missing_runtime_publishes_minimal_failed_gate(self) -> None:
        with tempfile.TemporaryDirectory(prefix="a10m5r10r1r4-missing-runtime-") as temporary:
            root = Path(temporary)
    ...
    def test_oversized_pre_python_failure_log_is_bounded(self) -> None:
        with tempfile.TemporaryDirectory(prefix="a10m5r10r1r4-large-log-") as temporary:
            root = Path(temporary)
            run_root = root / BUILDER.RUN_ID
            run_root.mkdir()
...
class CorpusExtractionRootTests(unittest.TestCase):
    ...
    def test_old_destination_nests_and_fixed_destination_exposes_corpus(self) -> None:
        with tempfile.TemporaryDirectory(prefix="a10m5r10r1r4-corpus-root-") as temporary:
            root = Path(temporary)
    ...
    def test_corrected_corpus_target_is_retained_in_both_wrappers(self) -> None:
        for name in ("run_control.sh", "run_candidate.sh"):
            source = (PACKAGE / "artifacts/jobs" / name).read_text(encoding="utf-8")
            self.assertEqual(
                source.count('tar -xf "$run_root/corpus.tar" -C "$job_local"'),
                1,
            )
            self.assertNotIn(
    ...
    def test_authority_preflight_rejects_unpinned_corpus(self) -> None:
        with tempfile.TemporaryDirectory(prefix="a10m5r10r1r4-corpus-pin-") as temporary:
            root = Path(temporary)
            archive = root / "corpus.tar"
            self.make_archive(archive)
            write(
    ...
    def test_authority_rejects_coordinated_pin_archive_manifest_drift(self) -> None:
        with tempfile.TemporaryDirectory(prefix="a10m5r10r1r4-pin-drift-") as temporary:
            root = Path(temporary)
...
class ScienceEnvironmentClosureTests(unittest.TestCase):
    PROHIBITED = ("LD_LIBRARY_PATH", "PYTHONHOME", "PYTHONPATH")

    def make_corpus(self, path: Path) -> None:
        with tarfile.open(path, "w:") as archive:
            payload = b"{}\n"
    ...
    def run_wrapper(self, name: str) -> dict[str, object]:
        with tempfile.TemporaryDirectory(prefix="a10m5r10r1r4-parent-env-") as temporary:
            root = Path(temporary)
            job_local = root / "job-local"
            environment = job_local / "runtime/environment"
            (environment / "bin").mkdir(parents=True)
            os.symlink(sys.executable, environment / "bin/python")
            wrapper = root / name
            shutil.copyfile(PACKAGE / "artifacts/jobs" / name, wrapper)
            wrapper.chmod(0o700)
            bootstrap = root / "bootstrap_environment.sh"
            bootstrap.write_text(
                "#!/bin/sh\n"
                "export CUBLAS_WORKSPACE_CONFIG=child-only\n"
                "export PATH=/child-only\n"
                "export PIP_CACHE_DIR=/child-only\n"
                "export PYTHONNOUSERSITE=child-only\n"
                "export TMPDIR=/child-only\n"
                "export TORCH_HOME=/child-only\n"
                "export XDG_CACHE_HOME=/child-only\n"
                "export PYTHONPATH=/child-only\n"
                "export PYTHONHOME=/child-only\n"
                "export LD_LIBRARY_PATH=/child-only\n",
                encoding="utf-8",
            )
            bootstrap.chmod(0o700)
            self.make_corpus(root / "corpus.tar")
            recorder = """import argparse,json,os
parser=argparse.ArgumentParser()
parser.add_argument('--output', required=True)
options,unused=parser.parse_known_args()
names=('CUBLAS_WORKSPACE_CONFIG','PATH','PIP_CACHE_DIR','PYTHONNOUSERSITE','TMPDIR','TORCH_HOME','XDG_CACHE_HOME')
value={'environment': {name: os.environ.get(name) for name in names}, 'prohibited_present': {name: name in os.environ for name in ('LD_LIBRARY_PATH','PYTHONHOME','PYTHONPATH')}}
path=os.path.join(options.output, 'science-environment.json')
os.makedirs(options.output, exist_ok=True)
with open(path + '.promote', 'w', encoding='utf-8') as stream:
    json.dump(value, stream, indent=2, sort_keys=True)
    stream.write('\\n')
os.replace(path + '.promote', path)
"""
            role = "control-materialization"
            if name == "run_control.sh":
                (root / "materialize_controls.py").write_text(
                ...
                output = root / "results/control-materialization"
            else:
                role = "monthly-residual-adapter-k1"
                (root / "candidate_experiment.py").write_text(
                    recorder, encoding="utf-8"
                )
                controls = root / "results/control-materialization"
                controls.mkdir(parents=True)
                write(controls / "control-summary.json", {"valid": True})
                write(controls / "evidence.json", {"valid": True})
                arguments = (
                    str(wrapper),
                    str(job_local),
                    role,
                ...
                output = root / f"results/{role}"
            ambient = {
                "CUBLAS_WORKSPACE_CONFIG": "ambient-wrong",
                "LD_LIBRARY_PATH": "/ambient/lib",
                "PATH": "/usr/bin:/bin",
                "PIP_CACHE_DIR": "/ambient/pip",
                "PYTHONHOME": "/ambient/home",
                "PYTHONNOUSERSITE": "ambient-wrong",
                "PYTHONPATH": "/ambient/python",
                "TMPDIR": "/ambient/tmp",
                "TORCH_HOME": "/ambient/torch",
            ...
            return json.loads(
                (output / "science-environment.json").read_text(encoding="utf-8")
            )

    def test_both_wrappers_restore_exact_environment_from_empty_export(self) -> None:
        for name in ("run_control.sh", "run_candidate.sh"):
            with self.subTest(name=name):
                value = self.run_wrapper(name)
                captured = value["environment"]
                job_root = str(Path(captured["TMPDIR"]).parent)
                self.assertEqual(
                    captured["PATH"],
                    f"{job_root}/runtime/environment/bin:/usr/bin:/bin",
                )
                normalized = dict(captured)
                normalized["PATH"] = "/registered/run/runtime/bin:/usr/bin:/bin"
                for key in (
                    "PIP_CACHE_DIR",
                    "TMPDIR",
                    "TORCH_HOME",
                    ...
                    self.assertEqual(captured[key], f"{job_root}/{suffix}")
                    normalized[key] = f"/registered/job-local/attempt/{suffix}"
                self.assertEqual(normalized, BUILDER.REQUIRED_JOB_ENVIRONMENT)
                self.assertEqual(
                    value["prohibited_present"],
                    {name: False for name in self.PROHIBITED},
                )

    def test_wrapper_delta_and_contract_are_exact(self) -> None:
        FREEZE.verify_two_file_science_environment_delta()
        remedy = json.loads(
            (PACKAGE / "artifacts/job-local-capacity-contract.json").read_text(
                encoding="utf-8"
            )
        )
        FREEZE.verify_science_environment_contract(BUILDER, remedy)

    def test_each_required_assignment_mutation_fails_guard(self) -> None:
        source = (PACKAGE / "artifacts/jobs/run_control.sh").read_text(
            encoding="utf-8"
        )
        for name in sorted(BUILDER.REQUIRED_JOB_ENVIRONMENT):
            assignment = next(
                line
                for line in source.splitlines()
                if line.startswith(f"export {name}=")
            )
            for replacement in ("", f"export {name}=mutated"):
                with self.subTest(name=name, replacement=replacement):
                    mutated = source.replace(assignment, replacement, 1)
                    with self.assertRaises(AssertionError):
                        FREEZE.verify_wrapper_environment_source(
                            mutated, BUILDER.REQUIRED_JOB_ENVIRONMENT
                        )
...
class PredecessorBindingTests(unittest.TestCase):
    ...
    def test_authority_and_plan_preserve_predecessor_and_admissions(self) -> None:
        with tempfile.TemporaryDirectory(prefix="a10m5r10r1r4-builder-") as temporary:
            root = Path(temporary)
            source_commit = self.source_commit()
            ...
            self.assertEqual(
                plan["required_job_environment"],
                BUILDER.REQUIRED_JOB_ENVIRONMENT,
            )
            self.assertIn(
                "asset-manifest.json",
