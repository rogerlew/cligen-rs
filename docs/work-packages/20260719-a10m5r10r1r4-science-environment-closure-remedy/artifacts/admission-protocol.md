...
`a10m5r10r1r4-science-environment-closure-remedy-r0`.
...
## Parent science-environment closure

`bootstrap_environment.sh` remains an executed child process, so none of its
exports are treated as parent state. Immediately after bootstrap succeeds,
both `run_control.sh` and `run_candidate.sh` clear `PYTHONPATH`, `PYTHONHOME`,
and `LD_LIBRARY_PATH`, reconstruct all seven values from the plan's
`required_job_environment`, and assert the exact values and prohibited-name
absence before invoking any parent-side Python. Wrapper values using
`$environment/bin` and `$job_local` normalize respectively to
`/registered/run/runtime/bin` and `/registered/job-local/attempt` for exact
comparison with the plan. `CUBLAS_WORKSPACE_CONFIG` must equal `:4096:8` before
the first deterministic PyTorch import. A missing, changed, inherited, or
late-applied value fails the supervised role before science.
...
## Corpus extraction contract

Before authority initialization, run `verify_corpus_layout.py` against the
materialized `corpus.tar` and `asset-manifest.json`. The verifier, asset
preparer, and authority/plan builder must first authenticate the layout pin
itself as exactly 864 bytes with SHA-256
`ea17be0acecc8afc99714c1c9df42511440382c2b8fb6d0511b0aad3414069d3`.
They then authenticate the
224,040,960-byte archive, SHA-256
`8770e127f8413eedd47d50670c359e450988444a8c4d8d43ca5645619a1b0a17`,
101 regular relative members, sole `corpus/` prefix, and all three required
manifests. Authority must not be created from an asset root that fails this
preflight.

Both control and candidate wrappers extract the archive into `$job_local`,
not `$job_local/corpus`. They then pass `$job_local/corpus` to the unchanged
science. A nested `$job_local/corpus/corpus` is invalid; do not add a fallback
or path selector.
