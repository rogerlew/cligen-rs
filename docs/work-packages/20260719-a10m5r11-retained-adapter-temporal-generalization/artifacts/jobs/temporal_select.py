...
def neural_streams(root: Path, roles: list[dict[str, Any]]) -> dict[str, dict[str, list[dict[str, float]]]]:
    output: dict[str, dict[str, list[tuple[int, int, dict[str, float]]]]] = defaultdict(lambda: defaultdict(list))
    for role in roles:
        document = read(root / "results" / role["role_id"] / "streams.json")
        if document.get("valid") is False or not document.get("streams"):
            raise RuntimeError(f"invalid neural stream role: {role['role_id']}")
        if document.get("configuration_id") != role["configuration_id"]:
...
def main() -> None:
    ...
    neural = neural_streams(options.neural_root, contract["roles"])
    ...
    result = {
        ...
        "schema_version": 1,
        "terminal": (
            "A10M5R11-TEMPORAL-READY"
            if eligible
            else "HOLD-A10M5R11-NO-TEMPORALLY-ELIGIBLE-CANDIDATE"
        ),
    }
