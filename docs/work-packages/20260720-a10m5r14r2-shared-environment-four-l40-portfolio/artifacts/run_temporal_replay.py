#!/usr/bin/env python3
"""Authenticate R14R2 portfolio evidence and run the inherited selector twice."""
...
def verify_portfolio_records(evidence_root: Path, plan: dict, collection: dict) -> dict:
    ...
    if len(rows) != 4 or not required <= allowlist or not required <= present:
        raise RuntimeError("R14R2 portfolio evidence roster incomplete")
    identities = {}
    ...
    if not (
        ...
        raise RuntimeError("R14R2 portfolio operational evidence authentication failed")
    for row in rows:
        ...
        if not (
            ...
            == "a10m5r14r2-submission-admission"
            ...
            raise RuntimeError(f"R14R2 child evidence authentication failed: {row['role']}")
        identities[row["role"]] = {
...
def main() -> None:
    ...
    if subprocess.run(("git", "show", f"{head}:{relative}"), cwd=REPO, check=True, capture_output=True).stdout != Path(__file__).read_bytes():
        raise RuntimeError("R14R2 replay differs from published source")
    ...
    if not (
        ...
        raise RuntimeError("R14R2 collection authentication failed before replay")
    ...
    replay["record_type"] = "a10m5r14r2-precleanup-replay"
