...
target=${TMPDIR:-/tmp}/a10m5r8-$role-$SLURM_JOB_ID
...
for name in ('comparison.json', 'decision.json', 'control-identity.json', 'treatment-training.json'):
...
if os.path.exists(partial):
    ...
    value = {'classification': 'a10m5r8-development-only-climate-statistics-objective', 'gates': {'experiment_published': False}, 'protected_roles_opened': []}
