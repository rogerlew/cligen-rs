#!/usr/bin/env python3
"""Prepare immutable A10M5O2 canonical multi-L40 assets."""
...
def wrapper(role: str, world: int, mode: str) -> str:
    return f"#!/bin/sh\nset -eu\nexec ./job-common.sh {role} {world} {mode}\n"
...
def main() -> None:
    ...
    generated = ["qualify.py", "rank_failure.py", "run-qualification.sh", "job-common.sh"]
    for name in generated:
        copy(jobs / name, root / name)
    ...
    roles = (
        ("single-baseline", 1, "success"),
        ("dual-qualification", 2, "success"),
        ("quad-qualification", 4, "success"),
        ("dual-rank-failure", 2, "failure"),
    )
    for role, world, mode in roles:
        name = f"job-{role}.sh"
        (root / name).write_text(wrapper(role, world, mode), encoding="utf-8")
        generated.append(name)
    recovery = {"invoked": False, "reason": "primary roles own supervised job-local cleanup"}
