...
def validate(source_commit: str | None = None) -> dict:
    ...
    if not (
        ...
        and actual == value.get("realized_l40_minutes") == 38
        and value.get("outstanding_study_ceiling_l40_minutes") == 515
        and value.get("bounded_maximum_l40_minutes") == actual + 515 == 553
        ...
        and len(value.get("released_recovery_reserves", [])) == 2
        and [row.get("kind") for row in components] == expected_kinds
        and len({row.get("evidence_path") for row in components}) == 4
        and len({row.get("record_sha256") for row in components}) == 4
    ):
    ...
    if not (
        len({row.get("evidence_path") for row in releases}) == 2
        and len({row.get("event_sha256") for row in releases}) == 2
    ):
