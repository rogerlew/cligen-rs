...
SUCCESSOR_RECONSTRUCTION = PACKAGE / "artifacts/successor-reconstruction-contract.json"
CONTROL_CALIBRATION = PACKAGE.parent / "20260722-a10m5r15r2r1-successor-control-identity-calibration"
CONTROL_IDENTITY = CONTROL_CALIBRATION / "artifacts/successor-control-identity.json"
CONTROL_COLLECTION = CONTROL_CALIBRATION / "artifacts/execution-r3-collection.json"
CONTROL_TERMINAL = CONTROL_CALIBRATION / "artifacts/execution-r3-terminal.json"
CAMPAIGN_ACCOUNTING = PACKAGE / "artifacts/campaign-accounting.json"
...
RUN_ID = "a10m5r15r2r5-pending-admission-remedy-r0"
PORTFOLIO_ROLE = "external-normal-conditioning-portfolio"
RECORD_TYPE = "a10m5r15r2r4-submission-admission"
CALIBRATION_GATES = {
...
def validate_successor_control(source_commit: str) -> None:
    identity_value = json.loads(CONTROL_IDENTITY.read_text(encoding="utf-8"))
    reconstruction = json.loads(SUCCESSOR_RECONSTRUCTION.read_text(encoding="utf-8"))
    collection = json.loads(CONTROL_COLLECTION.read_text(encoding="utf-8"))
    terminal = json.loads(CONTROL_TERMINAL.read_text(encoding="utf-8"))
    calibrated = reconstruction.get("successor_control_calibration", {})
    expected = []
    for row in reconstruction.get("models", []):
        item = dict(row)
        item.pop("non_gating_accepted_export_sha256", None)
        expected.append(item)
    if not (
        "Status: `A10M5R15R2R1-SUCCESSOR-CONTROL-IDENTITY-READY`"
        in (CONTROL_CALIBRATION / "package.md").read_text(encoding="utf-8")
        and digest(CONTROL_IDENTITY) == calibrated.get("identity_artifact_sha256")
        and identity_value.get("all_static_identities_exact") is True
        and identity_value.get("candidate_output_accessed") is False
        and identity_value.get("protected_roles_opened") == []
        and [row.get("actual") for row in identity_value.get("models", [])] == expected
        and authenticated(collection)
        and authenticated(terminal)
        and collection.get("record_sha256") == calibrated.get("collection_record_sha256")
        and terminal.get("record_sha256") == calibrated.get("terminal_record_sha256")
    ):
        raise RuntimeError("successor control calibration identity drift")
    for path in (CONTROL_CALIBRATION / "package.md", CONTROL_IDENTITY, CONTROL_COLLECTION, CONTROL_TERMINAL):
        if path.read_bytes() != git_bytes(source_commit, path):
            raise RuntimeError(f"successor calibration differs from published source: {path.name}")


def validate_campaign_accounting(source_commit: str) -> None:
    campaign.validate(source_commit)
...
def published(source_commit: str) -> None:
    ...
    for path in (
        ...
        PACKAGE / "artifacts/jobs/campaign_accounting.py",
...
def transform_contracts(output: Path, contract: dict) -> None:
    ...
    def portfolio(value):
        reconstruction = json.loads(
            SUCCESSOR_RECONSTRUCTION.read_text(encoding="utf-8")
        )
        controls = []
        for row in reconstruction["models"]:
            control = dict(row)
            control.pop("non_gating_accepted_export_sha256", None)
            controls.append(control)
        value["controls"]["models"] = controls
...
def install_scheduler_pending_admission(output: Path) -> None:
    """Make occupancy a recorded observation; Slurm verifies capacity at start."""
    path = output / "admission_checker.py"
    text = path.read_text(encoding="utf-8")
    old_parse = 'all(match is not None or "(null)" in line.lower() for line, match in zip(active, gpu_matches, strict=True))'
    new_parse = 'all(match is not None or "(null)" in line.lower() or "|n/a|" in line.lower() for line, match in zip(active, gpu_matches, strict=True))'
    if old_parse not in text or '"at_least_two_l40_idle": active_gpu_count <= 2,' not in text:
        raise RuntimeError("parent occupancy admission transform drift")
    text = text.replace(old_parse, new_parse, 1)
    text = text.replace(
        '"at_least_two_l40_idle": active_gpu_count <= 2,',
        '"scheduler_pending_submission": True,',
        1,
    )
    text = text.replace(
        '"record_type": "a10m5r14r2r2-immediate-pre-submit-occupancy",',
        '"record_type": "a10m5r15r2r5-scheduler-pending-occupancy",',
        1,
    )
...
"""
    ...
    replacements = {
        ...
        "child_records_ok = True": (
            ...
            "    and runtime.get('package_id') == '20260812-a10m5r15r2r5-pending-admission-remedy'\n"
...
def main() -> None:
    ...
    install_scheduler_pending_admission(options.output)
    ...
    overlays = {
        ...
        / "research/a10/lemhi_toolkit/remote/recover_job_local_v2.sh",
        "campaign-accounting.json": CAMPAIGN_ACCOUNTING,
        "campaign_accounting.py": PACKAGE
        / "artifacts/jobs/campaign_accounting.py",
    }
