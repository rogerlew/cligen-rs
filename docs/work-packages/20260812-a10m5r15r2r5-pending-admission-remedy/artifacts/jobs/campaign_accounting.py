...
def validate(source_commit: str | None = None) -> dict:
    ...
    for row in components:
        ...
        elif kind == "successful-r2r3-control":
            valid = (
                authenticated(evidence)
                and evidence.get("record_sha256") == row["record_sha256"]
                and evidence.get("job_id") == "1060872"
                and evidence.get("run_id")
                == "a10m5r15r2r3-portfolio-evidence-remedy-r0"
                and evidence.get("result", {}).get("state") == "COMPLETED"
                and evidence.get("result", {}).get("actual_gpu_minutes")
                == row["actual_l40_minutes"]
                == 19
            )
        elif kind == "failed-r2r3-e2-initialization":
            valid = (
                authenticated(evidence)
                and evidence.get("record_sha256") == row["record_sha256"]
                and evidence.get("job_id") == "1060874"
                and evidence.get("run_id")
                == "a10m5r15r2r3-portfolio-evidence-remedy-r0"
                and evidence.get("result", {}).get("state") == "FAILED"
                and evidence.get("result", {}).get("actual_gpu_minutes")
    ...
    if not (
        len({row.get("evidence_path") for row in releases}) == 4
        and len({row.get("event_sha256") for row in releases}) == 4
    ):
