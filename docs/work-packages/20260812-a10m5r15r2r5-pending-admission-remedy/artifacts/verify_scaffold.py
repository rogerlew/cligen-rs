...
R2R3 = PACKAGE.parent / "20260722-a10m5r15r2r3-portfolio-evidence-remedy"
...
if (
    pin["package_id"] != "20260722-a10m5r15r2r4-e2-initialization-remedy"
...
predecessor_core = (R2R3 / "artifacts/jobs/continuous_core.py").read_text(
    encoding="utf-8"
)
old_assignment = """                self.location_baseline.weight[:, list(DESCRIPTOR_COLUMNS)].copy_(
                    common.weight[list(LOCATION_HEAD_INDICES)]
                )"""
new_assignment = """                self.location_baseline.weight[:, list(DESCRIPTOR_COLUMNS)] = (
                    common.weight[list(LOCATION_HEAD_INDICES)]
                )"""
if old_assignment not in predecessor_core or core != predecessor_core.replace(
    old_assignment, new_assignment
):
    raise RuntimeError("R2R5 must change only the E2 indexed initialization write")
for token in (
    "EXPECTED_COUNTS",
    "normal_only_basis",
    "descriptor_basis",
    "nn.init.zeros_(self.climatology_heads.weight)",
    "self.location_baseline.weight[:, list(DESCRIPTOR_COLUMNS)] = (",
...
builder_spec = importlib.util.spec_from_file_location(
    "r2r4_build_control_records", PACKAGE / "artifacts/jobs/build_control_records.py"
)
if builder_spec is None or builder_spec.loader is None:
    raise RuntimeError("cannot load R2R5 authority builder")
