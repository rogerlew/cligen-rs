...
run_root=$PWD
output=$run_root/results/climate-normal-residual
...
"$environment/bin/python" "$run_root/residual_core.py"

stderr=$output/experiment.stderr.part
if ! "$environment/bin/python" "$run_root/experiment.py" \
  --run-root "$run_root" --corpus "$job_local/corpus" --output "$output" \
  2>"$stderr"; then
  sed "s|$run_root|[REMOTE_RUN_ROOT]|g; s|$job_local|[JOB_LOCAL]|g" "$stderr" >&2 || true
  exit 1
fi
rm -f -- "$stderr"
printf '%s\n' A10M5R9-EXPERIMENT-COMPLETE
