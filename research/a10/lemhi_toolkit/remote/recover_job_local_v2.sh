...
if [ ! -e "$target" ]; then
  ...
  umask 077
  printf '{"gates":{"job_local_cleanup":true,"marker_revalidated":true,"original_job_settled":true,"recovery_exact_node":true},"recovery_result":{"absence_mode":"already-absent","node":"%s","original_job_id":"%s","target_sha256":"%s"}}\n' \
